...
void post_process_thread() {
    ...
    pos_mtx << cos(M_PI / 4), 0.0, -sin(M_PI / 4),
                0.0, 1.0, 0.0,
    ...
    predictor_params.locator_params.trans_mtx = pos_mtx;
    ...
    // Commander
    COMMANDER_Params commander_params;
    commander_params.KP_4_aim = 5;
    commander_params.KI_4_aim = 0;
    commander_params.KD_4_aim = 0;
    commander_params.max_vel = 1024;
    commander_params.max_omega = 512;
    Commander commander(commander_params, predictor_params.locator_params);
    ...
    Messenger messenger(can_params, 80, 100);
    std::stringstream can_logger;
    messenger.activate(can_logger);
    ...
    while (!thread_exit) {
        ...
        if (ExposureRecorder<frame_data_mono>::instance().getLatestExposureData(latest)) {
                ...
                if(latest.detected)
                {
                    if(predictor.locate(std::get<0>(latest.bboxes), std::get<1>(latest.bboxes), std::get<2>(latest.bboxes), std::get<3>(latest.bboxes), locate_result))
                    {
                        ...
                        // 计算指令
                        commander.aim_iterate(std::get<0>(latest.bboxes), std::get<1>(latest.bboxes), std::get<2>(latest.bboxes), std::get<3>(latest.bboxes), latest.timestamp_ns, cmd);
                        std::cout<< "cmd: "<< cmd.vx << ' ' << cmd.vy << ' ' << cmd.omega << ' ' << cmd.omega << std::endl;
                    }
                }
                else
                {
                ...
                // 发送指令
                messenger.set_control_state(cmd.vx, cmd.vy, cmd.omega);
